...
.OUTPUTS
	Email Report
.NOTES
	NAME:	Monitor-ADGroupMembership.ps1
	AUTHOR:	Francois-Xavier Cat, Brian Steingraber
    ...
    2.1 2018.07.24
        Added new switch/logic to allow the emailto to use Group Manager
        Change Email Server, EmailTo, EmailFrom from Mandatory to Not required.
        Updated to use Send-MailMessage cmdlet

    2.2 2018.08.07
        Added code to allow sending emails to both the manager and pipeline input.
	...
	TODO:
        ...
        -Support both Computer and User membership
\
#>
...
PARAM (
	[Parameter(ParameterSetName = "Group", Mandatory = $false, HelpMessage = "You must specify at least one Active Directory group")]
	[ValidateNotNull()]
	...
	[Alias('SearchBase')]
	[String[]]$SearchRoot = "OU=Email,OU=Groups,OU=KSM,dc=kidney-mn,DC=local",
...
PROCESS
{
	TRY
	{
		...
		FOREACH ($item in $Group)
		{
			TRY
			{
				...
				IF ($GroupName)
				{
					...
					If ($Changes -or $AlwaysReport)
					{
                        ...
                        $Manager = $GroupName.managedby
                        #Get email of Manager
                        $ManagerEmail = Get-ADUser -Identity $Manager -Properties EmailAddress | Select-Object -ExpandProperty EmailAddress
                        $EmailSubject = "AD Email Group Monitoring - $($GroupName.SamAccountName) Membership Change"
                        #Combine any emails in Parameter and managers email
                        $recipients = $Emailto + $ManagerEmail
                        ...
                        $body += "<u>Group Manager:</u> $($GroupName.managedby)<br>"
                        $body += "<u>Manager Email:</u> $($ManagerEmail)<br>"
						...
						if(-not($OneReport)) {                      
                            ...
                            $emailParams = @{
                                To = $recipients
                                #To = $ManagerEmail
                                From = "kits-noreply@kidney-mn.com"
			...
			if($OneReport) {
				$EmailSubject = "AD Email Group Monitoring - Membership Report"
				#  Preparing the body of the Email
